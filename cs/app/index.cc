...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < 256; i++) {
    for (int j = 0; j < 256; j++) {
#ifdef TEST_SDL_LOCK_OPTS
      // Alpha behaves like in the browser, so write proper
      // opaque pixels.
      int alpha = 255;
#else
      // To emulate native behavior with blitting to screen,
      // alpha component is ignored. Test that it is so by
      // outputting data (and testing that it does get
      // discarded)
      int alpha = (i + j) % 255;
#endif
  ...
  printf(
      "you should see a smoothly-colored square - no sharp "
      "lines but the square borders!\n");
  printf(
      "and here is some text that should be HTML-friendly: "
      "amp: |&| double-quote: |\"| quote: |'| less-than, "
      "greater-than, html-like tags: "
      "|<cheez></cheez>|\nanother line.\n");
