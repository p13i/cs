...
int Parser_Tests_main(int argc, char **argv) {
  printf("Running Parser_Tests_Run() from parser_tests.cc\n");
...
{
    EXPECT_EVALUATION("add(add(1,1),add(1,add(1,1)))", true, 5);
}
...
{
    EXPECT_EVALUATION("sub(add(1,1),div(10,mul(1,2)))", true, -3);
}

TEST(Operator, Embedded7)
{
    EXPECT_EVALUATION("sub(add(1,1),div(9,mul(1,div(3,1))))", true, -1);
}
...
{
    EXPECT_EVALUATION("exe(var( add(1,1)),var(B ) , var( C) ) ", false, 0);
}
